...
export default function CartSidebar({ isOpen, onClose }: Props) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed top-0 right-0 h-full w-[360px] bg-white/50 backdrop-blur border-l border-white/20 z-50 shadow-2xl transform transition-transform duration-300
      ...
      >
        <div className="p-4 flex flex-col h-full">
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <button
              onClick={onClose}
              className="text-lg cursor-pointer bg-red-500 text-white px-2 py-0.5 rounded-md hover:bg-red-600 transition"
            >
              ✕
            </button>
          </div>
              ...
              cart.map((item) => (
                ...
                >
                  ...
                  <div className="flex flex-col gap-1 pr-6">
                    <p className="text-sm font-medium text-zinc-800 break-words leading-tight">
                      {item.title}
                    </p>
          ...
          {cart.length > 0 ? (
            <div className="mt-4 border-t pt-4 space-y-3">
              {/* Total */}
              <div className="flex justify-between items-center text-base font-semibold text-zinc-800">
                <span>Total</span>
      ...
      {toast && (
        <div className="fixed bottom-15 right-5 bg-black text-white px-4 py-2 rounded-lg shadow-lg text-sm z-[999]">
          {toast}
        </div>
      )}
    </>
