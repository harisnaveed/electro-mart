...
export default function CheckoutPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const res = await fetch("http://localhost/stripe-endpoint/create-checkout-session.php", {
        method: "POST",
        headers: {
        ...
        <div className="flex-1">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    title="Remove item"
                  >
                    <FontAwesomeIcon icon={faTrash} className="text-gray-500 text-sm hover:text-red-500" />
                  </button>
                </div>
              </div>
            )))}
          ...
          <div className="bg-white p-5 rounded-xl shadow-sm border">
            ...
            <button
              className="mt-2 w-full text-sm text-zinc-600 cursor-pointer hover:underline"
              onClick={() => navigate("/")}
            >
              ← Continue Shopping
            </button>
